...
enum editorKey {
    BACKSPACE = 127, // ASCII code for backspace since there's no special key like \n or \r etc
    ARROW_LEFT = 1000,
    ARROW_RIGHT,
    ARROW_UP,
    ARROW_DOWN,
    ...
    for (y = 0; y < E.screenrows; y++) {
     ...
     if (filerow >= E.numrows) {
        if (E.numrows == 0 && y == E.screenrows / 3) {
            char welcome[80];
            char welcomelen = snprintf(welcome, sizeof(welcome),
        ...
        for (j = 0; j < len; j++) {
            if (iscntrl(c[j])) {
                ...
                if (current_color != -1) {
                    char buf[16];
                    int clen = snprintf(buf, sizeof(buf), "\x1b[39m", current_color);
...
void initEditor() {
    ...
    E.screenrows -= 2; // Leave space for status bar and prompt
}
